    ...
    def timer_callback(self):
        ...
        if self.state == self.STATE_IDLE:
            ...
            try:
                ...
                telemetry = {
                    "imu": {
                        "roll": round(math.degrees(self.latest_euler[0]), 2),
                        "pitch": round(math.degrees(self.latest_euler[1]), 2),
                        "yaw": round(math.degrees(self.latest_euler[2]), 2),
                    },
                    "reward": 0.0,
                    "action": [round(float(v), 4) for v in target_q],
                    "battery": 12.3,
                    "mode": "walk",
                }
                with open("/tmp/tinker_telemetry.json", "w") as f:
